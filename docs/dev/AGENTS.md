...
- **Killing a backgrounded `git push` does not kill the gate it started.** The
  ...
  worktree whose push was interrupted, look for the survivor. `pgrep -af
  <worktree-path>` names `scripts/check_parallel.sh` and its chain subshells
  (forks share its argv): `make check` passes `$(CURDIR)` to it for exactly
  this purpose. A one-shot `pgrep` is exec'd in place of the shell and does not
