...
| Full gate | `make check` | every `lint` and `test` target, `webui-build`, the repo guards and the stub drift checks — the header of [`scripts/check_parallel.sh`](../../scripts/check_parallel.sh) lists each step; what pre-push and CI run |
