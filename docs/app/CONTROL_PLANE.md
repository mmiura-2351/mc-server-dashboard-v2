...
| Code | When |
|---|---|
| `SERVER_NOT_FOUND` | The target server is unknown to this Worker (no live instance: stop/restart/command on a not-running server, a missing file target, or a stopped-id snapshot whose working dir is absent — already GC'd after a published final snapshot, or never hydrated; issue #1713). Reserved for ids this Worker genuinely knows nothing about: a **failed-stop orphan** is not one of them and answers `INVALID_STATE` instead (issue #2466). |
| `INVALID_STATE` | The command is invalid for the current settled state: start or hydrate a running server, or a command over a server with a failed-stop orphan pending termination — start, hydrate, stopped-id snapshot, restart, console command and relay tunnel dial alike (issue #2466). The orphan is a process this Worker could not confirm dead, so it is probably still alive holding its port and writing its world; reporting it as not-running would tell the operator a live server is down. `StopServer` is the exception, by design: it *takes* the orphan and re-attempts the driver Stop, which is the only path that resolves it. |
