...
|---|---|---|---|
| `StartServer` | Launch the server (after a preceding hydrate). Carries the driver, JAR relpath, MC version (Worker picks the Java runtime), launch mode (`jar` is the default JAR launch; `forge-argsfile` launches Forge via its generated args file, running a supervised installer first when the working set is uninstalled), optional `memory_limit_bytes` (per-server memory ceiling; 0/unset = driver default), and optional `cpu_millis` (soft CPU allocation in millicores; 0/unset = driver default). The "after a preceding hydrate" is enforced, not assumed: a launch whose working set is not held at the scratch root is refused `SERVER_NOT_FOUND` rather than launched into an empty directory (Section 7). The check is the presence of the generation marker `.mcsd_generation`, so a scratch emptied in place is refused as well as an absent one, and it guards `RestartServer`'s relaunch too — both launches go through the same code path. | none | FR-SRV-2, FR-EXE-5, ARCHITECTURE.md Section 7.3 |
| `StopServer` | Stop the server; graceful stop triggers an event-driven snapshot. `force` skips graceful. | none | FR-SRV-2, FR-DATA-7 |
| `RestartServer` | Stop then start in place. The relaunch takes the same working-set guard as `StartServer`, so a restart over a scratch destroyed or emptied out of band is refused `SERVER_NOT_FOUND` once the stop has confirmed; the server is left down and the reconciler re-launches it with a hydrate (Section 7). | none | FR-SRV-2 |
...
| `SnapshotTrigger` | Push the working set back to the data plane (a running server's copy is bracketed save-off → async save-all → settle-wait → copy → save-on so the world is fully on disk and a region file is not captured torn — NOT `save-all flush`, whose synchronous main-thread flush can trip the server watchdog and crash a live server; a running snapshot that cannot quiesce — RCON down, save-off/save-all failing, or the save never settling — is refused `quiesce_unavailable` rather than packing a live world; a stopped-id snapshot whose scratch holds no working set — already GC'd after a published final snapshot, emptied out of band, never hydrated, or holding only the generation marker — is refused `SERVER_NOT_FOUND` rather than packing an empty tar with the base-generation guard disabled). | none | FR-DATA-4, FR-DATA-7, REQUIREMENTS.md Section 6.9 |
...
| Code | When |
|---|---|
| `SERVER_NOT_FOUND` | The target server is unknown to this Worker (no live instance: stop/restart/command on a not-running server, a missing file target, or a stopped-id snapshot whose scratch holds no working set — already GC'd after a published final snapshot, emptied out of band, or never hydrated; that predicate is the working set's CONTENT, so a directory holding only the generation marker is refused too, there being nothing to capture). On a LAUNCH it is the same statement about the working set rather than about an instance: the API issues a `HydrateTrigger` before every start that needs one, so a working set the Worker cannot show at launch means the API skipped the hydrate — its held-working-set inventory said this Worker holds a generation at least as fresh as the store — over a set the Worker does not actually hold. Launching would boot the server into an empty directory and the next snapshot would publish that, so the launch is refused instead. The predicate is the presence of the generation marker `.mcsd_generation` (exactly that name; the `.mcsd_generation-*` temps of an interrupted write do not count), not the existence of the directory: the marker is the Worker's held-claim token, so a scratch emptied in place has lost the claim the skipped hydrate relied on and is refused like an absent one. A directory holding ONLY the marker passes — that is what a 204 "nothing published yet" hydrate leaves, and booting a fresh world out of it is the contract. The guard covers both launches: on a `StartServer` the API answers it by re-launching WITH a full hydrate, and on a `RestartServer` it is reached on the relaunch, after the restart's own stop confirmed, so the server is left down and the reconciler's re-dispatched start takes that same replay — the recovery is stop → hydrate → start. All these cases carry one shared phrase in the message, which is how the API tells them from any other `SERVER_NOT_FOUND`; the phrase and the Worker's exact text are declared in the contract table's `messages` (Section 7.1), not restated here. Where a command's precondition is "this server is running", the code means the Worker holds neither an instance nor a **failed-stop orphan** for the id: the orphan answers `INVALID_STATE` or `BUSY` instead, per the split below. |
...
working-set-absent refusal of the `SERVER_NOT_FOUND` row above is the only
such match today, and until it was declared here the API's phrase and the
fixtures feeding it were hand copies of a Go literal nothing checked — a
Worker-side reword dropped the match with both suites green.
