...
| Code | When |
|---|---|
| `SERVER_NOT_FOUND` | The target server is unknown to this Worker (no live instance: stop/restart/command on a not-running server, a missing file target, or a stopped-id snapshot whose working dir is absent — already GC'd after a published final snapshot, or never hydrated; issue #1713). Where a command's precondition is "this server is running", the code means the Worker holds neither an instance nor a **failed-stop orphan** for the id: the orphan answers `INVALID_STATE` instead (issue #2466). |
| `INVALID_STATE` | The command is invalid for the current settled state: start or hydrate a running server, or a command over a server with a failed-stop orphan pending termination — start, hydrate, stopped-id snapshot, restart, console command, relay tunnel dial and Bedrock tunnel open alike (issue #2466). The orphan is a process this Worker could not confirm dead, so it is probably still alive holding its port and writing its world; reporting it as not-running would tell the operator a live server is down. Two commands are deliberately exempt: `StopServer` *takes* the orphan and re-attempts the driver Stop (the path that resolves it on demand), and `CloseBedrockTunnel` takes no running check at all, so a tunnel that outlived its server can still be torn down. Nothing has to ask, though: as of issue #2475 the Worker converges the orphan itself — it probes the instance's real liveness on a backoff, retries the stop while it is alive, retires the record and reports `stopped` once it is confirmed gone, and reports `SERVER_STATE_UNKNOWN` while the backend cannot answer. The record also still clears on its own if the process exits, via the instance's status pump. |
