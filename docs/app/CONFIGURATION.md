...
| `auth.token.refresh_cookie_name` | `mcd_refresh` | | Name of the `HttpOnly` refresh-token cookie: `/api/auth/login` always sets it (the sole refresh-token transport from login), `/api/auth/refresh` rotates it and `/api/auth/logout` clears it only when the request carried it, and `/api/auth/session` reads it without re-setting it (AUTH_API.md Section 3). Must be non-empty. |
