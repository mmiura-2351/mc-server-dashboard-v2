...
> and their defaults — not how configuration is loaded; the loaders fail fast
> on a bad config (Section 2).
...
starting in a half-configured state. The API also rejects an unknown key in the
config file, with one deliberate asymmetry: an unrecognised `MCD_API_*`
environment variable is silently ignored, because the environment is shared and
may legitimately carry unrelated prefixed names.
...
| Port (ref.) | Selector key | Adapter choices | Side |
|---|---|---|---|
...
- The `ExecutionDriver` Port is **pluggable** by design (FR-EXE-2): the seam
  admits other backends (e.g. a Kubernetes driver). One driver ships:
  `container`.
...
- The `Storage` adapter contract and the per-backend keys' full semantics are
  owned by STORAGE.md; this document fixes only the **selector** and the
  shape of the adapter-specific groups (Section 5.2).
- Other Ports in ARCHITECTURE.md Section 5 have a **single adapter** and so
  need no selector key (e.g. `TokenService`, `PermissionChecker`,
  `WorkerRegistry`, `Clock`); they are added here only when a second adapter
  exists. `TokenService` is a single "JWT-or-equivalent" adapter
...
|---|---|---|---|
| `server.host` | `0.0.0.0` | | Bind address for both listeners the API process hosts: the HTTP API (REST + data-plane endpoint) and the control-plane gRPC server. IPv4 or IPv6 literal. See the bind note below for the compose/container topology. |
| `server.http_port` | `8000` | | Port for the HTTP API. Must be 0..65535; `0` binds an OS-assigned ephemeral port. See the bind note below for the compose/container topology. |
...
| `server.data_plane_base_url` | `server.public_base_url` | | Base URL handed to Workers for hydrate/snapshot transfers instead of `server.public_base_url`. Defaults to `server.public_base_url`, so a split deployment with no edge proxy needs no separate setting. Set this on a deployment whose public URL routes through a body-size-capped edge (e.g. Cloudflare Tunnel, ~100 MB, see `DEPLOYMENT.md` Section 8) to an internal address instead, so a co-located Worker's working-set upload bypasses the edge rather than hairpinning through it and 413ing. A **cross-host** Worker must always set it explicitly — neither the compose-internal pin nor the public URL is right there (`DEPLOYMENT.md` Section 8). Because that omission otherwise only surfaces at transfer time, the API logs a startup `WARN` when this is unset while `server.public_base_url` is set; setting it explicitly — including to the same value as `server.public_base_url` — records the intent and silences the warning. |
...
| `control.tls.client_ca_file` | — | | **Reserved; not implemented.** Client-certificate (mTLS) verification of the Worker. Reserved, unused — the shared `control.worker_credential` authenticates the Worker (NFR-SEC-1) and the control channel is server-side TLS only. Documented so the config shape admits mTLS without renaming. |
...
³ **Reconciler grace floor (duplicate-start + stop-hold safety).** Keep
...
The stop-side `control.snapshot_timeout_seconds` is the SECOND term of
...
the hold exists to close, because a final-snapshot **timeout** also leans on the
...
The stop command's own `control.stop_timeout_seconds` is the THIRD
...
> **Local/dev configs.** A process with `control.enabled=true` fails fast at
> startup unless it supplies a cert/key pair or sets
> `control.tls.insecure=true`; add `control.tls.insecure=true` to local/dev
> configs.
...
| Key | Default | Secret | Meaning |
|---|---|---|---|
| `database.url` | *required* | secret | Connection string for the persistence adapter (may embed credentials). Model owned by DATABASE.md. |
| `database.pool_size` | `5` | | Number of permanent connections SQLAlchemy keeps open. Must be positive. Mirrors SQLAlchemy's own default, so an untuned deployment behaves as SQLAlchemy does out of the box. |
...
| `storage.object.connect_timeout_seconds` | `10` | | Per-operation TCP connect timeout for the object-store client (STORAGE.md Section 7.3). Explicit so a stalled connect fails fast rather than inheriting botocore's hidden 60s default. Must be positive. |
| `storage.object.read_timeout_seconds` | `60` | | Per-operation single-socket-read timeout for the object-store client. Bounds one streamed chunk, not a whole transfer, so a multi-GB hydrate/snapshot/backup is unaffected while a genuinely stalled read still fails within a known ceiling. Must be positive. |
| `storage.object.retry_max_attempts` | `5` | | Total attempts (initial + retries) the object-store client makes per operation under botocore's `standard` retry mode (capped exponential backoff), set explicitly rather than inheriting botocore's default retry mode. Must be at least 1 (`1` disables retries). |
...
publish behaviour (REQUIREMENTS.md FR-DATA-6) live in STORAGE.md.

**Connection-pool sizing and the lifecycle lock.** Every at-rest-gated
...
| `auth.token.refresh_reuse_grace_seconds` | `60` | | Grace window after a refresh token is rotated within which re-presenting the **predecessor** is treated as a legitimate concurrent refresh (two SPA tabs, or a retry of a refresh whose response was lost) rather than theft: a fresh pair is issued and the token family is kept. Outside the window the reuse still revokes the whole family (DENIED audit event). Must be positive. Larger values tolerate more clock skew / slower retries at the cost of a longer replay window for a leaked predecessor secret. |
...
| `auth.token.download_grant_ttl_seconds` | `30` | | Lifetime of a **download grant** — the short-lived token in the self-authenticating download URL a browser streams a multi-GB body from: a backup archive, a server export ZIP, or a file / directory ZIP (AUTH_API.md Section 3). One knob covers all three. Must be positive. Grants are TTL-only (not single-use), so this window is the whole exposure of one that leaks into a reverse-proxy access log; it only has to cover the round trip from minting the URL to the browser starting the download. |
| `auth.token.download_cookie_ttl_seconds` | `900` | | Lifetime of the **download cookie** a redeemed grant is exchanged for (AUTH_API.md Section 3) — the credential a browser's retry of an interrupted transfer authenticates with, once the short-lived grant in the retried URL has expired. Also its `Max-Age`. One knob covers all three downloads. Must be positive. Unlike a grant this never reaches an access log, a browser history or a `Referer` (`HttpOnly`, and no URL carries it), so it can be longer; a transfer that runs past this window still cannot resume. The default matches `access_ttl_seconds` deliberately: logout cannot clear per-resource path-scoped cookies, so this is the window in which a leftover cookie on a shared browser re-reads its one resource, and at the default that is not longer than an access token the same user already holds. Raise it for genuinely slow links, knowingly widening exactly that window. |
| `auth.token.download_cookie_secure` | `true` | | Sets the `Secure` flag on the download cookie (HTTPS-only). Turn it **off** only for plain-HTTP localhost development — otherwise the browser stores nothing and every resume silently reverts to the 401 the cookie exists to prevent. |
...
Scheduled backups (REQUIREMENTS.md FR-BAK-3) are not a standalone loop: a backup
is a `backup` schedule on the general scheduler (Section 5.14 and
DATABASE.md Section 8), so `schedule.tick_seconds` is the only cadence knob.
This section has no keys.

Nor is backup cadence a per-server config key: `backup_interval_hours` is not a
config key, and a server create/update carrying it is rejected with 422
`retired_config_key` (Section 8).
...
| `reconciler.grace_seconds` | `660` | | How long a divergence must persist (measured from the last Worker report) before it is acted on, so the normal in-flight lifecycle path has time to converge first. Must be positive and `> max(control.hydrate_timeout_seconds + control.command_timeout_seconds, control.snapshot_timeout_seconds, control.stop_timeout_seconds)` (the start round-trip budget, the held stop-snapshot budget, and the stop dispatch budget); below the floor the reconciler can re-dispatch a first start before it settles (risking a duplicate live instance), clear a still-healthy final-snapshot hold mid-upload (reopening the stop→re-place race), or replay a stale stop before its first round-trip settles — a `WARN` fires on boot. The stock default (660) satisfies the stock floor (`max(600 + 30, 600, 600) = 630`). |
| `reconciler.held_start_grace_seconds` | `90` | | The SHORTER grace applied only to a `redispatch_start` whose assigned Worker is connected AND already holds a fresh-enough working set, so the start skips hydrate and is command-only. The long hydrate-based `grace_seconds` is pure dead waiting there, and the cross-worker duplicate-live-instance race cannot occur on a re-dispatch to the same already-connected Worker (its double-start guard rejects a second live start). All other paths keep the full `grace_seconds`. Must be positive and `> control.command_timeout_seconds` (the start command round-trip budget — the hydrate budget in the `grace_seconds` floor does not apply to a command-only start); below the floor the reconciler can re-dispatch a held start before its command round-trip settles and race the in-flight start — a `WARN` fires on boot. The stock default (90) satisfies the stock floor (30). |
| `reconciler.refused_stop_grace_seconds` | `30` | | The SHORTER grace applied only to a `redispatch_stop` whose last stop dispatch the Worker **refused**. The only term of the `grace_seconds` floor that governs this path is `> control.stop_timeout_seconds` (never replay a stop whose first dispatch may still be running), and a returned refusal *is* that round trip ending — so the protection is satisfied by the answer rather than by waiting, and it has **no floor of its own**. Without it a server the operator asked to stop keeps running, and accepting players, for ~11-12 min after a refusal the API already knows about. `BUSY` does not count as a refusal (it is the Worker reporting that something else *is* in flight for the id), the refusal must still be the most recent thing known about the row (a newer intent commit or Worker report supersedes it), and it is not persisted — a stop divergence with no recorded refusal, including every one that survives an API restart, keeps the full `grace_seconds`. Must be positive. |
...
An operator may instead supply an explicit `game_port` in the
...
(`server.bedrock_port`), allocated from a dedicated window — an
...
Operator-configurable memory-limit default and ceiling for per-server
allocations. Both default to unset (the built-in defaults apply). Surfaced to
clients via `GET /api/meta` so the create wizard can pre-fill and cap the value.

| Key | Default | Secret | Meaning |
|---|---|---|---|
| `memory_limit.default_mb` | *unset* | | Application-wide default memory allocation (MiB) for new servers when the create request omits `memory_limit_mb`. `None` (unset) leaves the request blank (driver default). Must be at least 512 MiB; must not exceed `max_mb` when both are set; must not exceed the built-in ceiling (1 TiB) when `max_mb` is unset. |
| `memory_limit.max_mb` | *unset* | | Operator-configurable ceiling that overrides the built-in 1 TiB maximum. `None` (unset) keeps the built-in ceiling. Must be at least 512 MiB. |
...
The compose `api` image builds the SPA and sets this to
...
`server_plugin` row references. It is gated on the control plane
...
session prune loop. `relay.credential` and `relay.base_domain` are both
...
The API runs the general-scheduler runner, which polls
...
| `api.tls.client_cert_file` | — | | **Reserved; not implemented.** Path to the Worker's client certificate for an mTLS control channel. The API does not verify client certificates (server-side TLS only; `api.credential` authenticates the Worker), so this key is reserved, unused; it is documented so the config shape admits mTLS without renaming. Must be set together with `api.tls.client_key_file` (or neither); a half-configured pair fails config validation, naming the missing half. |
...
The concrete capability message on the wire is defined in `proto/`; this
...
| `driver.container.docker_host` | *(daemon default)* | | Docker daemon endpoint when the `container` driver is enabled. Only a `unix://` socket is supported; empty uses the daemon's default socket. |
| `driver.container.images` | *(empty)* | | Map of Java **major version** to the base container image providing that JRE; the `container` driver picks the image matching a server's Minecraft version by the version→major bracket table below (REQUIREMENTS.md FR-EXE-5, ARCHITECTURE.md Section 7.3). **Required** when `worker.drivers` advertises `container`. See below. |
...
maps a server's Minecraft version to a required Java major
(`javaruntime.MajorsFor`, ARCHITECTURE.md Section 7.3) and selects the image for
it; a version with no configured image fails the launch. The brackets:
...
the deliberate stateless-worker posture (the Worker adopts no prior state; the
...
limit. **An unset limit leaves the heap at the JVM default.** The limit is also
a **hard ceiling** the process cannot exceed:

| Driver | Memory guarantee | Mechanism |
|---|---|---|
| `container` | **Hard ceiling — enforced.** The process cannot exceed the limit; the kernel OOM-kills it if it tries. | Docker `Memory` set to the limit, plus the derived `-Xmx`. |
...
|---|---|---|
| `container` | **Soft relative share — enforced under contention only.** The allocation sets the server's CPU weight proportional to other containers; when the host is otherwise idle the server may burst above it. There is **no hard cap**. | Docker `CPUShares`, proportional to the allocation. |

These guarantees cover memory and CPU; disk quotas are deferred, not implemented
(REQUIREMENTS.md Section 2.3).
...
Log streaming is **transient relay-only** (REQUIREMENTS.md Section 6.13):
...
reverse-proxy trust). [SECURITY.md](SECURITY.md) specifies the semantics of
each knob; this section carries the keys and their defaults.

### 7.1 Password policy

Password strength is selected by a named preset rather than per-rule toggles.
The preset fixes which strength rules fire and their thresholds;
...
The default is `middle`. Because the policy only validates *newly set*
passwords, changing the preset leaves existing hashes unaffected; operators who
want the stricter posture set `auth.password.policy=high`.

The `auth.password` group accepts only `policy` and `max_length` (this
section) and `hash` (Section 5.3). Any other key — a per-rule toggle such as
`auth.password.min_length` or `auth.password.require_complexity`, or its
`MCD_API_AUTH__PASSWORD__*` env form — fails fast with a startup validation
error; the preset is the only strength selector.
...
These knobs cover the abuse surface FR-AUTH-4's authentication hardening does
not: a master switch to close self-registration on an admin-provisioned
...
Raise `ip_threshold` or widen who is trusted (Section 7.3) where a deployment
expects shared egress.
...
(REQUIREMENTS.md FR-DATA-7; REQUIREMENTS.md Section 9, decision 4).
Configuration covers the two periodic dimensions; event-driven snapshots are
behavioural, not configured.
...
- **Event-driven snapshots** — taken on graceful stop and on-demand backup
  (REQUIREMENTS.md FR-DATA-7 and REQUIREMENTS.md Section 6.11) regardless of the
  interval; they have no configuration key.
...
Scheduled backups do not follow this config-key pattern: a backup is a `backup`
schedule on the general scheduler (DATABASE.md Section 8), polled at
`schedule.tick_seconds` (Section 5.14) like every other schedule.
`backup_interval_hours` is not a config key; a server create/update carrying it
is rejected with 422 `retired_config_key`.
