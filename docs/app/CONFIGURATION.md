...
| `auth.token.download_grant_ttl_seconds` | `30` | | Lifetime of a **download grant** — the short-lived token in the self-authenticating download URL a browser streams a multi-GB body from: a backup archive, a server export ZIP, or a file / directory ZIP (AUTH_API.md Section 3, issues #2313, #2352). One knob covers all three. Must be positive. Grants are TTL-only (not single-use), so this window is the whole exposure of one that leaks into a reverse-proxy access log; it only has to cover the round trip from minting the URL to the browser starting the download. |
| `auth.token.download_cookie_ttl_seconds` | `900` | | Lifetime of the **download cookie** a redeemed grant is exchanged for (AUTH_API.md Section 3, issue #2373) — the credential a browser's retry of an interrupted transfer authenticates with, once the short-lived grant in the retried URL has expired. Also its `Max-Age`. One knob covers all three downloads. Must be positive. Unlike a grant this never reaches an access log, a browser history or a `Referer` (`HttpOnly`, and no URL carries it), so it can be longer; a transfer that runs past this window still cannot resume. The default matches `access_ttl_seconds` deliberately: logout cannot clear per-resource path-scoped cookies, so this is the window in which a leftover cookie on a shared browser re-reads its one resource, and at the default that is no longer than an access token the same user already holds. Raise it for genuinely slow links, knowingly widening exactly that window. |
