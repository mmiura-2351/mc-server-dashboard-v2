...
| `auth.token.download_grant_ttl_seconds` | `30` | | Lifetime of a backup **download grant** — the short-lived token in the self-authenticating download URL a browser streams a multi-GB archive from (AUTH_API.md Section 3, issue #2313). Must be positive. Grants are TTL-only (not single-use), so this window is the whole exposure of one that leaks into a reverse-proxy access log; it only has to cover the round trip from minting the URL to the browser starting the download. |
