...
- Session reporting hangs off this same flow lifecycle (issue #1904): `Lookup` /
  `Create` count each client->worker datagram, and once a flow crosses
  `flowPromoteThreshold` (a fixed const) it is reported to the API as a live
  session; idle eviction and tunnel teardown End it. See RELAY.md Section 8 for
  the full semantics and the accepted caveats.
...
- ~~**Bedrock session reporting** (issue #1904)~~ **Landed with #1904**: a
  Bedrock flow that crosses `flowPromoteThreshold` client->worker datagrams is
  reported to the API through the shared `session.Reporter` (RELAY.md
  Section 8), landing a `game_session` row like a Java login. Identity is null
