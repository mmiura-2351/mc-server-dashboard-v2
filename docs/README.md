...
| Doc | What it covers |
|---|---|
| [`app/ARCHITECTURE.md`](app/ARCHITECTURE.md) | The Hexagonal (Ports & Adapters) layering, the `proto/` / `api/` / `worker/` / `relay/` / `webui/` module boundaries and dependency direction, the catalog of domain Ports per side, the execution-backend, file-access, and JAR/Java decisions that [`REQUIREMENTS.md`](REQUIREMENTS.md) Section 9.1 delegates to design, and the API framework stack. |
| [`app/AUTH_API.md`](app/AUTH_API.md) | The `/auth/*` endpoint contract: per-endpoint status codes (a refresh with no token in either transport is a uniform 401; a logout with no token is an idempotent 204), the cookie-only `/auth/session` bootstrap, the RFC 9457 problem+json error shape and auth reason codes, body-vs-cookie transport precedence and `Set-Cookie` gating, which responses declare `Cache-Control: no-store` and why, the short-lived download grants and the download cookie a redemption sets (the third credential kind, for browser downloads), the refresh reuse grace window with Web UI session guidance, the CSRF posture, audit events, and the `/users/me/sessions` session-management endpoints. |
...
| [`app/CONFIGURATION.md`](app/CONFIGURATION.md) | Runtime configuration for `api/`, `worker/`, and `relay/` (the relay binary's own key table is in [`app/RELAY.md`](app/RELAY.md)): sources and precedence, secret handling, config-driven adapter selection for the `Storage`, `PasswordHasher`, and `ExecutionDriver` Ports, the authentication-hardening knobs and defaults, and snapshot-cadence settings. |
| [`app/CONTROL_PLANE.md`](app/CONTROL_PLANE.md) | The API↔Worker control-plane contract: the single gRPC bidirectional-stream service, message envelopes and correlation IDs, its connect/register/heartbeat/reconnect lifecycle, the command and event messages, error reporting, and how each maps to the requirements. The binding contract is the `proto/` buf module. |
...
| [`app/SECURITY.md`](app/SECURITY.md) | Authentication-hardening behaviour for `REQUIREMENTS.md` FR-AUTH-4: password-policy semantics, the brute-force/lockout algorithm, trusted-proxy client-IP resolution, and the decision on where the brute-force/lockout runtime state lives (DB-backed, behind a Port). Also the exposure posture for `/api/healthz`, `/api/readyz`, and the separate Prometheus listener, plus the Minecraft-server-container trust model: the two-network split that keeps user-supplied plugins off the control plane, and what it deliberately leaves open. |
...
| Doc | What it covers |
|---|---|
| [`dev/CONTRIBUTING.md`](dev/CONTRIBUTING.md) | The change workflow from issue discovery and implementation planning through branch naming, commits, pull requests, review hygiene, squash-merge, and issue closure. |
...
| [`dev/TESTING.md`](dev/TESTING.md) | The test-driven development discipline (Kent Beck): the red/green/refactor cycle, working disciplines, Tidy First, what a good test looks like, and the testing pyramid and feedback loops. Concrete tooling is per-component (`make test`; see the module READMEs). |
| [`dev/RELEASING.md`](dev/RELEASING.md) | Repository-wide SemVer versioning, tag naming and version authority, generated release notes, the tag-driven release and hotfix workflows, and the open decisions about artifact publishing and automated version bumping. A release does not build component artifacts — deployment builds them from the checked-out revision. |
| [`dev/DEPENDENCIES.md`](dev/DEPENDENCIES.md) | Pinning style, the 7-day supply-chain cooldown, security-update handling, the automated-update policy across every ecosystem in the repository, and the CI image pins Dependabot does not reach (PostgreSQL, SeaweedFS), re-pinned by hand under the same cooldown. |
...
| [`dev/DEPLOYMENT.md`](dev/DEPLOYMENT.md) | Single-host Docker Compose deployment: the core `db` / `api` / `worker` services and database-migration / object-lifecycle one-shots; the `object` (SeaweedFS), `relay`, and recommended `tunnel` (`cloudflared`) profiles; `.env` setup; bring-up and first-run admin bootstrap; how Minecraft server ports reach clients; TLS guidance for the browser and control planes; and the upgrade, backup, export/import, relay, and Bedrock procedures. |
