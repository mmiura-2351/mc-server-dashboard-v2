...
| [`dev/DEPENDENCIES.md`](dev/DEPENDENCIES.md) | Pinning style, the 7-day supply-chain cooldown, security-update handling, the automated-update policy across the Python, Go, and Node ecosystems, and the CI image pins Dependabot does not reach (PostgreSQL, SeaweedFS), re-pinned by hand under the same cooldown. |
