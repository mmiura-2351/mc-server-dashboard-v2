...
| [`app/CONFIGURATION.md`](app/CONFIGURATION.md) | Runtime configuration for `api/`, `worker/`, and `relay/` (the relay binary's own key table is in [`app/RELAY.md`](app/RELAY.md)): sources and precedence, secret handling, config-driven adapter selection (Storage backend, token service, execution drivers), the authentication-hardening knobs and defaults, and snapshot-cadence settings. |
