...
| [`dev/AGENTS.md`](dev/AGENTS.md) | Agent-facing operational manual complementing `CONTRIBUTING.md`: the primary-checkout / live-deploy ground rules, worktree lifecycle, the commands that fail silently (`git checkout <path>`, `--no-verify`, self-matching `pgrep -f`, `uv run --active`), gh/account quirks, and the pre-PR checklist. |
