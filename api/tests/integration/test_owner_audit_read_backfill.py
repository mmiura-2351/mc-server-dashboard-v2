"""Round-trip for the 0008 backfill of ``audit:read`` onto preset Owner roles.

Runs only when ``MCD_TEST_DATABASE_URL`` is set (the CI Postgres service);
skipped otherwise (TESTING.md Section 5). Seeds an old-style preset Owner role
(its permission array missing ``audit:read``) at revision 0007, then upgrades to
0008 and asserts the code was appended; also asserts the migration leaves a
non-preset role and a non-Owner preset role untouched, and that downgrade strips
the code from the preset Owner role again (issue #131).
"""
...
# An old-style Owner permission set (every community code *except* audit:read).
...
async def _seed(conn: AsyncConnection, community_id: uuid.UUID) -> dict[str, uuid.UUID]:
    ...
    rows = [
        # The target: a preset Owner role missing audit:read.
...
async def test_backfill_adds_audit_read_to_preset_owner_only() -> None:
    assert _DB_URL is not None
    await downgrade_base(_DB_URL)
    await upgrade_to("0007_audit_log", _DB_URL)
    ...
    try:
        ...
        # Apply the backfill.
        await upgrade_to("0008_owner_audit_read_backfill", _DB_URL)

        async with engine.connect() as conn:
            assert "audit:read" in await _perms(conn, ids["owner"])
            assert "audit:read" not in await _perms(conn, ids["custom"])
            assert "audit:read" not in await _perms(conn, ids["other_preset"])

        # Idempotent: re-running upgrade adds no duplicate.
        await downgrade_to("0007_audit_log", _DB_URL)
        await upgrade_to("0008_owner_audit_read_backfill", _DB_URL)
        async with engine.connect() as conn:
            owner_perms = await _perms(conn, ids["owner"])
        assert owner_perms.count("audit:read") == 1

        # Downgrade strips it from the preset Owner role again.
        await downgrade_to("0007_audit_log", _DB_URL)
        async with engine.connect() as conn:
            assert "audit:read" not in await _perms(conn, ids["owner"])
    finally:
