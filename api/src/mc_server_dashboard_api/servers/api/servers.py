...
async def create_server(
    ...
    except RetiredConfigKeyError as exc:
        # The request carried a retired platform key (``backup_interval_hours``,
        # #1840): the backup cadence moved to the general scheduler.
        raise _unprocessable("retired_config_key") from exc
...
async def update_server(
    ...
    **Game port (issue #311).** A new ``game_port`` is at rest only, validated
    ...
    try:
        ...
        # The PATCH carried a retired platform key (``backup_interval_hours``,
        # #1840): the backup cadence moved to the general scheduler.
        raise _unprocessable("retired_config_key") from exc
    except InvalidMemoryLimitError as exc:
        # A per-server memory limit outside the accepted shape/range (#705).
