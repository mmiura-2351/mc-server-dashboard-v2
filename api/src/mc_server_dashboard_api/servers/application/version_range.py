"""Decide whether a concrete mod version satisfies a dependency's range (#1293).

The Phase-C foundation: a pure, stdlib-only predicate the validator (#1263) uses
to turn a *present-but-wrong-version* dependency into a distinct finding, rather
than treating any present id as satisfied. No I/O, no DB, no network.

Two range dialects, selected by the depending **mod's** loader:
...
def version_satisfies(version: str, range_spec: str, loader: str) -> bool:
    ...
    ``loader`` is the depending mod's ``loader_type`` (``fabric``/``quilt`` ->
    semver predicates, ``forge``/``neoforge`` -> Maven intervals). An empty/any
    range, or any range that cannot be parsed in the loader's dialect, returns
    ``True`` (the documented tolerant fallback). Never raises.
    """
