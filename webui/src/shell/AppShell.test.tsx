...
describe("AppShell mobile drawer toggle (#583)", () => {
  it("renders a menu toggle button that opens and closes the sidebar", async () => {
    signedInWith([ALPHA, BETA]);

    renderAt("/");

    // The menu toggle is rendered with the "Open menu" label initially.
    const toggle = await screen.findByRole("button", {
      name: t("shell.menuOpen"),
    });
    expect(toggle).toBeInTheDocument();

    // The sidebar should not have the "open" class initially.
    const sidebar = toggle.closest(".shell")?.querySelector(".sidebar");
    expect(sidebar).not.toHaveClass("open");

    // Click to open.
    fireEvent.click(toggle);

    // After clicking, the sidebar gains the "open" class and the button label
    // switches to "Close menu".
    expect(sidebar).toHaveClass("open");
    expect(
      screen.getByRole("button", { name: t("shell.menuClose") }),
    ).toBeInTheDocument();

    // Click again to close.
    fireEvent.click(screen.getByRole("button", { name: t("shell.menuClose") }));

    expect(sidebar).not.toHaveClass("open");
    expect(
      screen.getByRole("button", { name: t("shell.menuOpen") }),
    ).toBeInTheDocument();
  });
  ...
  it("clicking a nav link closes the drawer", async () => {
    ...
    // Wait for the community data to load so the Dashboard link appears.
    const dashLink = await screen.findByRole("link", {
      name: t("nav.dashboard"),
    });
    ...
    // Click a nav link — the drawer should close.
  ...
  it("clicking the backdrop closes the drawer", async () => {
    ...
    // Open the drawer.
    fireEvent.click(toggle);
    const shell = toggle.closest(".shell");
    ...
    // Click the backdrop — the drawer should close.
