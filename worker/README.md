...
| `worker.drivers` | `MCD_WORKER_WORKER_DRIVERS` | yes | `container` is the only shipped driver (the host-process driver was removed in issue #781) and it requires `driver.container.images`, so there is no zero-config default: the set must be supplied and non-empty. |
