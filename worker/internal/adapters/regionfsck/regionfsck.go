...
// CheckRegionFile structurally validates one .mca region file. It returns
// ReasonNone if the file is structurally sound, or the first Reason that fails.
// A non-nil error is returned only on a real I/O failure. It reads at most the
// 8 KiB header plus a 5-byte prefix per present chunk; region payloads are never
// loaded.
func CheckRegionFile(path string) (Reason, error) {
	...
	for index := 0; index < entryCount; index++ {
		...
		if offset+sectorCount > totalSectors {
			return ReasonSectorOutOfBounds, nil
		}
		...
		// It must be positive and fit within the declared sectors (the 4-byte length
		// itself is not counted by the length field).
		if length < 1 {
