...
type Instance interface {
	...
	//
	// preFallback, when supplied, is called on the RCON-failure fallback path
	// (after tryRCONStop fails, before docker stop / SIGTERM) so the caller can
	// flush the live world to disk while the process is still alive. It is NOT
	// called when RCON stop succeeds (MC's own shutdown save already flushed),
	// nor on a force stop (graceful=false). At most one callback is accepted.
